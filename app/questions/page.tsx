...
export default function QuestionsPage() {
  ...
  async function handleDelete(e: React.MouseEvent, id: string, topic: string) {
    e.preventDefault()
    e.stopPropagation()

    const confirmed = window.confirm(`"${topic}" 문제 세트를 정말 삭제하시겠습니까?\n삭제하면 되돌릴 수 없습니다.`)
    if (!confirmed) return
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        alert(data.error || '삭제 중 문제가 발생했어요.')
        return
      }
          ...
          {filteredAndSorted.map((item) => {
            ...
            return (
              <Link
                ...
                className="relative block rounded-lg border border-gray-200 bg-white p-4 hover:bg-gray-50"
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-2">
                    {stats.dominantDifficulty && (
                      <span
                        className={`rounded px-2 py-1 text-[11px] font-medium ${getDifficultyBadgeClass(
                          stats.dominantDifficulty
                        )}`}
                      >
